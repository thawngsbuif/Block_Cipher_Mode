...
#define MAX_CIPHERTEXT_LEN  (MAX_PLAINTEXT_LEN + OFB_BLOCK_SIZE)
...
int main(void)
{
    ...
    unsigned char decrypted[MAX_PLAINTEXT_LEN + OFB_BLOCK_SIZE];
    int ct_len, pt_len;

    printf("=== AES-128 OFB mode (PKCS#7) ===\n\n");
    ...
    if (choice[0] == '1') {
        // ASCII
        printf("\nNhap plaintext (ASCII), vd: Hello OFB!\n");
        printf("> ");
    ...
    // --- Encrypt OFB+PKCS7 ---
    ct_len = ofb_encrypt_pkcs7(key, iv,
                               plaintext_bytes,
                               plaintext_len,
                               ciphertext);
    if (ct_len < 0) {
        printf("Loi: ofb_encrypt_pkcs7 that bai.\n");
        return 1;
    }
    ...
    // --- Decrypt để kiểm tra ---
    pt_len = ofb_decrypt_pkcs7(key, iv,
                               ciphertext,
                               ct_len,
                               decrypted);
    if (pt_len < 0) {
        printf("Loi: ofb_decrypt_pkcs7 that bai (padding sai?)\n");
        return 1;
    }
